...
class tienda
{
  ...
  function getCategorias()
	...
	//función para buscar el producto dado por el buscador.
	//en caso de que exista algún producto, se recogerá la información
	//seleccionada para poder interactuar sin problemas a la hora de querer agregarlo al carrito
	function buscarProductoPorNombre($nombre) {
		$sql = "SELECT CodProd, NombreProd, Stock, Precio, CodCat FROM producto WHERE NombreProd LIKE :nombre";
	...
	//función para añadir los productos almacenados en $cesta, al carrito
	function addCarrito($cesta, $carrito)
	{
		for ($i = 0; $i < sizeof($cesta); $i++) {
			$encontrado = false;
	...
	function ImprimirCarrito($carrito)
	{
		...
		//Validamos variable $total e imprimimos carrito, sumando los precios, para el subtotal
		//producto por unidades
		//y el sumatorio de los productos por sus unidades, dándonos el total
		$total = 0;
	...
	//calculamos directamente el total para pago.php
...
//funcion para confirmar el pedido
	...
	//Imprimimos los datos exactos del pedido al que se haya accedido
