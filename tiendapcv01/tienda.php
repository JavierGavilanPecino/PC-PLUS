...
//funcion para confirmar el pedido
	...
	//funcion para imprimir datos pedidos
	function ImprimirDatosPedidos($Codpedido){
		...
		for($i=0;$i<sizeof($pedidos);$i++)
		{
			...
			$sql_script2="SELECT NombreProd FROM producto WHERE CodProd=$Codprodvar ";
			$resultado2 = $this->ejecuta_SQL($sql_script2);
			$nombreproductos=$resultado2->fetchAll(\PDO::FETCH_ASSOC);
			echo "<tr>
			<td>".$pedidos[$i]['CodPed']."</td>
			<td>".$nombreproductos[0]['NombreProd']."</td>
			<td>".$pedidos[$i]['Unidades']."</td>
		</tr>";
		}
		echo "</table>";
	}
...
//funcion test para combrobar el usuario y clave
function test_comprobar()
{
    $shop = new tienda();
    $result = $shop->comprobar_usuario("Foster","clave3");
	//muestra el usuario y clave obtenidos
    print_r($result);
}

//funcion test para comprobar las categorias
function test_getCategorias()
{
	$shop = new tienda();
    $result = $shop->getCategorias();
	//muestra las categorias obtenidas
    print_r($result);
}

//funcion test para comprobar los productos
function test_getProductos()
{
	$shop = new tienda();
    $result = $shop->getProductos(1);
	//muestra los productos obtenidos
    print_r($result);
}

//funcion test para comprobar el nombre de la tienda
function test_getNombreCliente()
{
	$shop = new tienda();
    $result = $shop->getNombreCliente(1);
	//muestra el nombre de la tienda obtenido
    print_r($result);
}

//funcion test para comprobar el stock
function test_comprobarStock()
{
	$shop = new tienda();
	//el producto tiene 20 de stock para el ejemplo
	$result = $shop->comprobarStock(21,2,7);
	//muestra el stock obtenido
	echo $result;
}
?>